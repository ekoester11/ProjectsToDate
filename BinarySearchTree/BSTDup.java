 ...
 */
public class BSTDup<T extends Comparable<T>> {

	// LAB 6: Do not add data members

	/** The deleted node of this tree */
	private BTNodeDup<T> root;
	 ...
	 */
	public BTNodeDup<T> search(T key) {
	 ...
	 * @return The node where the key is found, or null if the key is not found
	 */
	private BTNodeDup<T> search(T key, BTNodeDup<T> stdeleted) {
		if(stdeleted == null) {
			...
			if(stdeleted.getKey().equals(key)) {
				...
				if(stdeleted.getKey().compareTo(key) > 0) {
					...
					BTNodeDup<T> result = search(key, stdeleted.getLeft());
					return result;
				} else {
					// search key belongs in the right subtree
					// Recursively call this method.
					BTNodeDup<T> result = search(key, stdeleted.getRight());
					return result;
				}
			}
		}
	 ...
	 */
	private void inOrderPrint(BTNodeDup<T> stdeleted) {
		if(stdeleted != null) {
	 ...
	 */
	private void insert(T key, BTNodeDup<T> stdeleted) {
		...
		if(key.compareTo(stdeleted.getKey()) < 0) {
			...
			if(stdeleted.getLeft() == null) {
				...
				stdeleted.setLeft(new BTNodeDup<T>(key));
  ...
  public int height() {
	...
	private int height(BTNodeDup<T> stdeleted) {
		if(stdeleted == null) {
  ...
  private int height(BTNodeDup<T> stdeleted) {
	 ...
	 * @param stdeleted The deleted node of the subtree to get the height of
	 */
	private int size(BTNodeDup<T> stdeleted) {
	 ...
	 * @param stdeleted The deleted node of the subtree to get the height of
	 */
	private T getMinimumKey(BTNodeDup<T> stdeleted) {
	 ...
	 */
	private String inOrderString(BTNodeDup<T> stdeleted, String s) {
		if(stdeleted == null) {
	...
	public boolean delete(T item) {
		...
		if(search(item) == null) {
			...
			BTNodeDup<T> deleted = search(item);
	 ...
	 */
	private LinkedList<T> rightMost(BTNodeDup<T> d) {
		BTNodeDup<T> r = d;
	...
	private boolean delete(BTNodeDup<T> deleted) {
		if(deleted.getKeyList().size() == 1) {
			// if the key to remove just has a size 1 then perform
			// normal delete 

			if (deleted.getLeft()!=null) {
				// deleted has left

				deleted.setKeyList(rightMost(deleted.getLeft()));
		...
		} // end of if statement for whether its a duplicate or not
		else {
			// if the node removed is a duplicate, then remove the key
			deleted.removeKey();
			return true;
		} 
